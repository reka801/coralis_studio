...
$routes->group('user', ['namespace' => 'App\Controllers'], function ($routes) {
    ...
    $routes->add('store', 'UserController::store');
    $routes->add('login', 'UserController::login');
    $routes->add('processLogin', 'UserController::processLogin');
    $routes->add('landing/(:num)', 'UserController::landing/$1');
    $routes->add('logout', 'UserController::logout');
    $routes->add('forgot-password', 'UserController::forgotPassword');
    $routes->add('reset-password/(:any)', 'UserController::resetPassword/$1');

    $routes->post('send-reset-link', 'UserController::sendResetLink');
    $routes->get('update-password/(:any)', 'UserController::updatePassword/$1');
    $routes->post('submit-update-password/(:any)', 'UserController::submitUpdatePassword');
});
